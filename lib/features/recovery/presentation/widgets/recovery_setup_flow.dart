...
import 'package:sabi_wallet/features/recovery/presentation/screens/contact_picker_screen.dart';
import 'package:sabi_wallet/features/recovery/services/nostr_invite_service.dart'
    hide ContactWithStatus;
import 'package:sabi_wallet/features/recovery/services/social_recovery_service.dart';
import 'package:sabi_wallet/features/recovery/presentation/screens/social_recovery_success_screen.dart';
...
class RecoverySetupFlow extends ConsumerStatefulWidget {
  ...
  const RecoverySetupFlow({required this.masterSeed, super.key});
    ...
    try {
      ...
      final recoveryContacts =
          contacts.map((contact) {
            return RecoveryContact(
              ...
              publicKey:
                  contact.hexPubkey ?? _generateDummyPublicKey(contact.name),
            );
          }).toList();
      ...
      for (final contact in contacts) {
        if (!contact.isOnNostr) {
          ...
          final tempKeypair =
              await NostrInviteService.generateTemporaryKeypair();
      ...
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              const CircularProgressIndicator(color: Color(0xFFF7931A)),
