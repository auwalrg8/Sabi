...
const String backgroundPaymentSyncPeriodicTask =
...
Future<void> _performBackgroundSync() async {
  ...
  try {
    ...
    if (nostrPubkey == null || fcmToken == null) {
      debugPrint(
        '⚠️ [Background] Missing nostr pubkey or FCM token, skipping sync',
      );
      return;
    }
    ...
    if (response.statusCode == 200) {
      ...
      final hoursSinceUse =
          (DateTime.now().millisecondsSinceEpoch - lastAppUse) /
          (1000 * 60 * 60);
      ...
      if (hoursSinceUse > 24) {
        ...
        debugPrint(
          '📭 [Background] User inactive for ${hoursSinceUse.toStringAsFixed(1)} hours - showing reminder',
        );
      }
    } else {
      debugPrint(
        '⚠️ [Background] Server response: ${response.statusCode} - ${response.body}',
      );
    }
  } catch (e) {
...
class BackgroundPaymentSyncService {
  static final BackgroundPaymentSyncService _instance =
