...
async function sendPushToUser(
  ...
  try {
    ...
    const tokens: string[] = pubkeyDoc.data()?.tokens || [];
    if (tokens.length === 0) {
      return 0;
    }
    ...
    functions.logger.info(
      `Push sent to ${nostrPubkey.substring(0, 8)}...: ` +
 ...
 */
async function cleanupInvalidTokens(nostrPubkey: string, tokens: string[]): Promise<void> {
  try {
    // Remove from pubkey index
    const pubkeyRef = db.collection("pubkey_devices").doc(nostrPubkey);
    await pubkeyRef.update({
 ...
 */
export const breezPaymentWebhook = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
 ...
 */
export const p2pTradeWebhook = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
 ...
 */
export const zapWebhook = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  ...
  try {
    ...
    const zapperName = senderName || senderPubkey?.substring(0, 8) || "Someone";
 ...
 */
export const dmWebhook = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  ...
  try {
    ...
    const senderDisplay = senderName || senderPubkey?.substring(0, 8) || "Someone";
 ...
 */
export const vtuWebhook = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
...
export const cleanupStaleTokens = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      const batch = db.batch();
      const tokensToRemove: { pubkey: string; token: string }[] = [];
